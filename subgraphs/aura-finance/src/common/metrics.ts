...
export function updateUsageMetricsAfterDeposit(block: ethereum.Block): void {
...
export function updateUsageMetricsAfterWithdraw(block: ethereum.Block): void {
  const usageMetricsDailySnapshot = getOrCreateUsageMetricsDailySnapshot(block);
  const usageMetricsHourlySnapshot =
    getOrCreateUsageMetricsHourlySnapshot(block);
...
export function updateVaultSnapshots(
  ...
  const vaultDailySnapshots = getOrCreateVaultDailySnapshots(vault.id, block);
  const vaultHourlySnapshots = getOrCreateVaultHourlySnapshots(vault.id, block);
...
export function updateRevenue(
  ...
  const financialMetrics = getOrCreateFinancialDailySnapshots(block);
  const vaultDailySnapshot = getOrCreateVaultDailySnapshots(vault.id, block);
  const vaultHourlySnapshot = getOrCreateVaultHourlySnapshots(vault.id, block);
...
export function updateRewards(
  ...
  const rewardToken = getOrCreateToken(rewardTokenAddr, block.number);
